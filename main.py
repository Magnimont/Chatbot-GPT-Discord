...
# Define 'bot' before using it as the command_prefix
...
async def generate_response(message):
    try:
        ...
        def call_openai_api():
            ...
            # Include the pre-prompt message with strict prohibition
            pre_prompt = {
                "role": "user",
                "content": "You are Maya, a polite and helpful assistant. Please note that you should not refer to yourself as 'ChatGPT.'"
            }
            ...
            return openai.ChatCompletion.create(
                ...
                messages=[
                    pre_prompt,
                    *get_messages(message.author.name, 'assistant', message.content)
                ]
            )
...
async def on_ready():
    ...
    activity_name = config["presence"]
    
    await bot.change_presence(status=idle_status, activity=discord.Activity(type=discord.ActivityType.watching, name=activity_name))
